...
def Uncorrelated_Entropy(timeseries, base=2):
    ...
    if length <= 1:
        return 0
    value, counts = np.unique(timeseries, return_counts=True)
    probs = counts / length
    entropy = 0.
    for i in probs:
...
def Conditional_Entropy(timeseries1, timeseries2, con_type='Shannon'):
    """ Calculates Conditional entropy of s1 based on s2
    Input:
        timeseries: list of strings or numbers,
            e.g. ['1', '2', '3'] or [1, 2, 3]
    Output:
        value of Conditional Entropy, e.g. 1.59
    """
    d = dict()
    for i in range(len(timeseries2)):
        d[timeseries2[i]] = d.get(timeseries2[i], []) + [timeseries1[i]]
    print(d)
    if con_type == 'Real':
        return sum([Real_Entropy(d[i]) * len(d[i]) / float(len(timeseries2)) for i in d])
    return sum([Uncorrelated_Entropy(d[i]) * len(d[i]) / float(len(timeseries2)) for i in d])
...
def get_predictability(timeseries, timeseries2=[], entropy_type = 'real', con_type='Shannon'):
    ...
    if entropy_type == 'condition':
        return maximum_predictability(N, Conditional_Entropy(timeseries, timeseries2, con_type))
