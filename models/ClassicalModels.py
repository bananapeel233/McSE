...
def ARIMA(data=None, params=None):
    ...
    :return: 1-d array, Forecasts in next h time steps.
    """
    model = auto_arima(y=data, max_p=params['maxp'], max_q=params['maxq'], start_q=params['startq'], start_p=params['startp'])
    return model
...
def regression(data=None, params=None):
    if params['model'] == 'SVR':
        model = svm.SVR(kernel=params['kernel'], max_iter=params['max_iter'])
